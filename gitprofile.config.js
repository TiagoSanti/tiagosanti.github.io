  ...
  externalProjects: [
    {
      ...
      description:
        'My research aims to help computer vision models learn new classes while retaining previously learned ones, recognizing all learned classes without task identity at inference time. I propose and implement ANT (Avoid Non-essential Tuning), a contrastive-learning intervention that selectively adjusts relationships among nearby negative examples. Integrated into TagFex, the method was evaluated on CIFAR-100, Tiny-ImageNet, and CUB-200. In the results presented at my qualification examination, the ANT-FS-GR variant with RefDetach improved average incremental accuracy across all five evaluated protocols.',
      details:
        'Contribution: formulation and implementation of an auxiliary margin term on negative similarities in InfoNCE, applied to both current-stage representation learning and contrastive distillation of prior knowledge. I investigate global versus per-anchor references, intra-view versus fully symmetric coverage, and the effect of detaching the reference from gradient computation (RefDetach). InfoNCE remains intact; ANT adds a selective adjustment signal.\n\nExperimental evaluation: five protocols across three datasets, three random seeds per configuration, and 243 runs used in the results tables. Comparisons retain the TagFex/ResNet-18 architecture and include ablation studies, average incremental accuracy, Nearest Mean of Exemplars (NME) classification, and forgetting. Configurations, seeds, and logs are recorded for traceability.\n\nResults presented on September 9, 2026: ANT-FS-GR with RefDetach improved average incremental accuracy by 0.11–0.65 percentage points over the InfoNCE baseline across the five protocols. On CUB-200 20–20, a different variant, ANT-IV-GR with RefDetach, achieved a gain of 1.29 percentage points. Values are means over three seeds. This is ongoing research: gains and forgetting vary across protocols, and the statistical ranking presented for four protocols does not establish significant superiority over the baseline.',
    },
    {
