...
test('static metadata uses canonical profile without depending on GitHub', () => {
  const result = staticSEO(config).transformIndexHtml.handler(
    '<title>Portfolio</title>'
  );
  assert.equal(result.html, '<title>Portfolio | Tiago Santi</title>');
  assert.equal(
...
test('static title escapes markup characters', () => {
  ...
  }).transformIndexHtml.handler('<title>Portfolio</title>');
  assert.equal(
    result.html,
    '<title>Portfolio | &lt;script&gt;&amp;&quot;</title>'
  );
});
